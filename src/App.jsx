...
.svc-btn{border:1.5px solid var(--border);border-radius:12px;padding:12px 12px;cursor:pointer;background:transparent;text-align:left;width:100%;transition:all .2s;display:flex;flex-direction:column;gap:3px;}
...
.o-stats-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;margin-bottom:24px;}
.o-stat{background:var(--card);border-radius:14px;padding:18px;border:1px solid var(--border);cursor:pointer;transition:all .2s;}
.o-stat:hover{border-color:var(--gold);transform:translateY(-2px);}
.o-stat-lbl{font-size:10px;color:var(--muted);text-transform:uppercase;letter-spacing:1.3px;}
.o-stat-val{font-family:'Cormorant Garamond',serif;font-size:36px;color:var(--ink);margin-top:2px;}
...
.res-actions{display:flex;gap:8px;margin-top:14px;padding-top:14px;border-top:1px solid var(--border);flex-wrap:wrap;}
...
@media(max-width:600px){
  .svc-grid{grid-template-columns:1fr 1fr;}.f-grid{grid-template-columns:1fr;}
      ...
      {showLogin&&(
        <div style={{position:"fixed",inset:0,background:"#1C0F2ECC",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,backdropFilter:"blur(6px)"}}>
          <div className="login-card">
            ...
            <div className="f-group" style={{marginBottom:12,textAlign:"left"}}>
            ...
            <div className="f-group" style={{marginBottom:18,textAlign:"left"}}>
  ...
  if(booking){
    return(
      <div style={{minHeight:"100vh",background:"var(--cream)"}}>
        ...
        <div className="bk-body" style={{paddingTop:16}}>
          {step<5&&(<div className="steps-bar" style={{marginTop:16}}>{[1,2,3,4].map((s,i)=>(<span key={s} style={{display:"contents"}}><div className={`step-node ${step===s?"active":step>s?"done":"idle"}`}>{step>s?"✓":s}</div>{i<3&&<div className="step-line"/>}</span>))}</div>)}
          {step===1&&(
            <div className="a4">
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",padding:"20px 20px 48px",flex:1}}>
        ...
        {(profile.instagram||profile.tiktok||profile.whatsapp||profile.phone||profile.email)&&(
          <div className="social-bar" style={{marginTop:32}}>
            {profile.instagram&&(
              <a className="social-link" href={`https://instagram.com/${profile.instagram.replace("@","")}`} target="_blank" rel="noreferrer" style={{color:"#E1306C"}}>
                <IGIcon/>
              </a>
            )}
            {profile.tiktok&&(
              <a className="social-link" href={`https://tiktok.com/@${profile.tiktok.replace("@","")}`} target="_blank" rel="noreferrer" style={{color:"#000"}}>
                <TTIcon/>
              </a>
            )}
            {profile.whatsapp&&(
              <a className="social-link" href={`https://wa.me/${profile.whatsapp.replace(/\D/g,"")}`} target="_blank" rel="noreferrer" style={{color:"#25D366"}}>
                <WAIcon/>
              </a>
            )}
            {profile.phone&&(
              <a className="social-link" href={`tel:${profile.phone}`} style={{color:"var(--gold)"}}>
                <PhoneIcon/>
              </a>
            )}
            {profile.email&&(
              <a className="social-link" href={`mailto:${profile.email}`} style={{color:"var(--ink)"}}>
                <MailIcon/>
              </a>
            )}
          </div>
        )}
        <div style={{marginTop:20,fontSize:10,color:"var(--border)"}}>© {new Date().getFullYear()} Elixir Beauty</div>
      </div>
    </div>
  )
}
    ...
    <div className="o-shell">
      ...
      {menuOpen&&(
        <div style={{position:"fixed",inset:0,zIndex:200}} onClick={()=>setMenuOpen(false)}>
          <div style={{position:"absolute",top:60,right:0,background:"var(--plum)",width:230,borderRadius:"0 0 0 14px",padding:"10px 0",boxShadow:"0 8px 32px #1C0F2E50"}} onClick={e=>e.stopPropagation()}>
            {[
      ...
      <div className="o-main">
        {view==="home"&&(
          <>
            ...
            {pending.length>0
              ?<><div style={{fontFamily:"Cormorant Garamond,serif",fontSize:19,color:"var(--ink)",marginBottom:12}}>{oLang==="fr"?"En attente":"Pending Requests"}</div><BookingList bookings={pending} onRespond={onRespond}/></>
